...
### 第一章 以用户体验为中心的性能指标
这一章我们将回到原点，抛开各种性能优化的实践，探寻性能优化是如何从0到1逐步建立的。笔者看到过各种所谓的性能优化书籍和文章，大多千篇一律地直接进入性能优化的实践。即便是好一些的文章，也只会简单讲述一些方法论。手握性能优化的锤子，看哪里都是钉子，最终导致为了优化而优化的KPI式指标。
...
- [2.1 DMAIC结构化性能分析方法](./packages/建立用户为中心的性能指标/2.1%20结构化性能分析方法.md): 利用DMAIC优化性能，确定关键瓶颈及其原因

- [2.2 抛弃盲目性能优化，建立用户评价模型](./packages/建立用户为中心的性能指标/2.2%20建立用户体验满意度模型.md): 如何一步步建立基于用户评价体系的满意度模型

- [2.3 体验数据采集与关联](./packages/建立用户为中心的性能指标/2.3%20体验数据采集与关联.md)：主动挖掘数据，分析数据，提出优化建议

- [2.4 如何建立有效的性能评分模型](./packages/建立用户为中心的性能指标/2.4%20如何建立有效的性能评分模型.md): 基于Lighthouse的评分模型原理，定制自己的评分模型

- [2.5 全局视角和用户动线](./packages/建立用户为中心的性能指标/2.5%20全局视角和用户动线.md)：如何使用统计学方法处理和分析性能数据，以便优化性能指标。
